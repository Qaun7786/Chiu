...
local Window = OrionLib:MakeWindow({Name = "Rimus Hub Auto Bounty", HidePremium = false, SaveConfig = true, ConfigFolder = "Rimus Hub Auto Bounty Config"})
--- Counter
local MainTab = Window:MakeTab({
	Name = "Main",
	Icon = "rbxassetid://15184124237",
	PremiumOnly = false
})
MainTab:AddButton({
	Name = "Skip Player",
...
MainTab:AddButton({
	...
	Callback = function()
        setclipboard("https://discord.gg/rimushub")
  	end    
})
Earn = MainTab:AddParagraph("Earned","0")
Total = MainTab:AddParagraph("Total Earned","0")
Elapsed = MainTab:AddParagraph("Time Elapsed","00:00:00")
...
SettingTab:AddToggle({
	Name = "Use Sword Method (equip both Melee and Sword)",
...
SettingTab:AddToggle({
	Name = "Use Gun Method (equip both Melee and Gun)",
...
function Notify(content)
    ...
    OrionLib:MakeNotification({
        Name = "Rimus Hub Auto Bounty",
        Content = content,
        Image = "rbxassetid://15184124237",
        Time = 5
    })
end
--- Tween + Bypass
...
function equip(tooltip)
    ...
    for _, v in pairs(player.Backpack:GetChildren()) do
        if v:IsA("Tool") and v.ToolTip == tooltip then
            ...
            if humanoid and not humanoid:IsDescendantOf(v) then
...
game:GetService("CoreGui").RobloxPromptGui.promptOverlay.ChildAdded:Connect(function(child)
    if not hopserver and child.Name == 'ErrorPrompt' and child:FindFirstChild('MessageArea') and child.MessageArea:FindFirstChild("ErrorFrame") then
        ...
        Notify("Rimus Hub Auto Bounty | Rejoin from kicked!")
    end
end)
--- Webhook
function wSend(main)
